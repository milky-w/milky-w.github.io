...
枚举每一条非树边找两个顶点树上路径的最大边（如果最大边与非树边边权相同则找次大边）然后更新最小增量。最大边和次大边可以通过树上倍增求出。
...
int head[N], nxt[N<<1], fa[N], dep[N], f[N][18], mx[N][18], tot, one, two;
...
void dfs(int cur, int fa, int deep) {
    ...
    for (int i = 1; (1 << i) < deep; ++i)
        f[cur][i] = f[f[cur][i-1]][i-1], mx[cur][i] = max(mx[cur][i-1], mx[f[cur][i-1]][i-1]);
    for (int i = head[cur]; i; i = nxt[i])
        if (g[i].to != fa) mx[g[i].to][0] = g[i].cost, dfs(g[i].to, cur, deep + 1);
}
void update(int x) {
    if (x > one) two = one, one = x;
...
void lca(int x, int y) {
    ...
    for (int i = 17; i >= 0; --i)
        if (dep[f[x][i]] >= dep[y]) update(mx[x][i]), x = f[x][i];
    if (x == y) return;
    for (int i = 17; i >= 0; --i)
        if (f[x][i] != f[y][i])
            update(mx[x][i]), update(mx[y][i]), x = f[x][i], y = f[y][i];
    update(mx[x][0]), update(mx[y][0]);
}
